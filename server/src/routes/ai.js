...
function callDeepSeek(systemPrompt, userPrompt, maxTokens) {
  return new Promise((resolve, reject) => {
    ...
    const url = new URL(config.AI_API_ENDPOINT.replace(/\/$/, '') + '/chat/completions');
    const options = {
...
router.post('/autocontent', (req, res) => {
  ...
  callDeepSeek(systemPrompt, `课程概要：${description}\n请扩展成约200字的课程内容汇报：`, 400)
    .then(({ content, usage }) => {
...
router.post('/performance', (req, res) => {
  ...
  callDeepSeek(systemPrompt, userPrompt, 250)
    .then(({ content, usage }) => {
